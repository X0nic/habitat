...
pub fn router(config: Arc<Config>) -> Result<Chain> {
    let router = router!(
        ...
        post "/jobs/:project_id" => XHandler::new(job_create).before(vec![Authenticated]),
        ...
        get "/projects/:id" => project_show,
        put "/projects/:id" => XHandler::new(project_update).before(vec![Authenticated]),
        delete "/projects/:id" => XHandler::new(project_delete).before(vec![Authenticated]),
    );
