...
pub fn project_delete(req: &mut Envelope,
                      ...
                      state: &mut ServerState)
                      -> Result<()> {
    let msg: proto::ProjectDelete = try!(req.parse_msg());
    // JW TODO: handle db errors
    // try!(state.datastore.projects.delete(msg.get_id()));
...
pub fn project_get(req: &mut Envelope,
                   ...
                   state: &mut ServerState)
                   -> Result<()> {
    let msg: proto::ProjectGet = try!(req.parse_msg());
    match state.datastore.projects.find(&msg.get_id()) {
...
pub fn project_update(req: &mut Envelope,
    ...
    match state.datastore.projects.update(&msg.get_project()) {
        Ok(()) => try!(req.reply_complete(sock, &msg)),
        Err(e) => {
